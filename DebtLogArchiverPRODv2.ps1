...
$source = @(Get-ChildItem "D:\Temp" | where{((Get-Date)-$_.LastWriteTime).days -ge 5} |Where-Object{$_.Name -like "*General*" -or $_.Name -like "*Api*" -or $_.Name -like "*Incoming*" -or $_.Name -like "*Outgoing*"} | Copy-Item -Destination "C:\Users\lafferic\Desktop\TempTest")
#Temporary Destination Directy. This is were the files are zipped from after being copied from D:\Temp
$files = "D:\Temp\TempCache"
...
Get-ChildItem "D:\Temp\TempCache" | Remove-Item -Recurse 
